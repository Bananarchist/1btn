module Controls exposing (updateControls, updateControlCmds, controllerSubs)

import Msg exposing (Msg(..))
import Model exposing (Model)
import Helpers exposing (concatIf, buildl, allPredicates)
...
updateControls msg model = 
    if listeningControlsか model then
        case msg of
            ...
            Msg.MouseReleased -> mouseDepressed model
            Msg.LostVisibility -> stopListeningForControls model
            _ -> model
    else 
        case msg of 
            Msg.ReceivedVisibility -> listenForControls model
            _ -> model
...
mousePressed = Model.setMouse True
mouseDepressed = Model.setMouse False

listenForControls = Model.setListeningControls True
...
updateControlCmds msg model = 
    case msg of
        Msg.Frame _ -> if listeningPadか model then [ Msg.pollPad ] else [ Cmd.none ]
        _ -> [ Cmd.none ]
